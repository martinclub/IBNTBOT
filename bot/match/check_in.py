...
class CheckIn:
	...
	async def start(self, ctx):
		...
		emojis = [self.READY_EMOJI, '🔸', self.NOT_READY_EMOJI, self.MENTION_EMOJI] if self.allow_discard else [self.READY_EMOJI, self.MENTION_EMOJI]
	...
	async def process_reaction(self, reaction, user, remove=False):
		...
		elif str(reaction) == self.MENTION_EMOJI:
			await self.mention_not_ready(bot.SystemContext(self.m.queue.qc))	

	async def mention_not_ready(self, ctx):
		not_ready = list(filter(lambda m: m not in self.ready_players, self.m.players))
		if len(not_ready):
			await ctx.reply(" ".join([f"<@{p.id}>" for p in not_ready]) + " please check-in to the match.")
